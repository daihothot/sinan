...
client auth 握手同样只能接受该 client_id / account_id 配置的 ACTIVE / NEXT secret，并必须继续绑定 terminal_id / platform / remote identity；日志和 Debug 输出不得包含 token 或配置 secret
...
```text
migration 文件命名：V0001__init.sql、V0002__state_store_schema.sql、V0003__execution_durability.sql、V0004__reconciliation_durability.sql、V0005__gateway_delivery_durability.sql、V0006__event_stream_sequence.sql、V0007__inbound_durability.sql
...
SQLite `PRAGMA user_version` 必须同步写入并校验当前最高 migration version，`schema_migrations` 是权威记录；当前最高版本为 V0007，因此完整迁移后的 `user_version = 7`，两者不一致时拒绝启动。
```
...
当前 `TransportEvent` 类型尚不携带 raw frame、message type 或 schema evidence，所以 TransportEvent adapter 写入的 `deadletter_events.raw_payload / raw_payload_length / message_type / schema_version` 可能为空。当前 schema 已提供这些持久化列和脱敏 summary 边界，但不能据此宣称完整 raw evidence 已经落库；真实 listener 投产前必须补齐有界 evidence 的传递。
...
V0007 应用后当前 schema 共有 30 张业务表（不含 `schema_migrations`）：V0002 创建 22 张，V0003 新增 1 张，V0004 新增 4 张，V0007 新增 3 张；V0005/V0006 只重建或扩展既有表，不增加最终表数。三张 admission 表只提供 durable intake 与 recovery 状态，不改变事实表和领域 projection 的 owner transaction 边界。
...
当前 Rust 第八里程碑已经实现 V0006/V0007 repository、durable admission/resume ports、lease recovery dispatcher 和 TransportEvent fact/summary adapter，但 TransportEvent 的 raw/type/schema evidence 仍未贯通；`sinan-core` 二进制也尚未启动 Native TCP / Execution WebSocket listener 或 dispatcher，并且尚未提供上述 handler-specific 业务 transaction。因此当前 `PENDING` row 是可靠 intake，不是已完成的 inbound processing。
...
已完成并在 workspace 中组合
  ...
  → V0006 stream_sequence 与 V0007 inbound/resume admission durability
  → Gateway production admission/resume/TransportEvent persistence ports；TransportEvent raw/type/schema evidence 尚未贯通
  → durable Circuit Breaker 在 HTTP bind 前 restore
  → HTTP/Event WS server 与 bounded event retention task

领域/Store API 已有，但当前 sinan-core main 尚未组合
  → Native TCP / Execution WebSocket listener 与 GatewayConnectionService
  → session registry / outbound delivery runtime lifecycle
  → DurableRecoveryDispatcher 的常驻 worker loop

下一里程碑必须实现
  → trusted single-snapshot RiskRequest assembler
  → TradeIntent accepted 后的 Risk→Execution workflow processor
  → command.received / execution.event / reconciliation.result 的 handler-specific 原子 transaction
  → listener 到 deadletter 的有界 TransportEvent raw/type/schema evidence
  → Gateway listener、dispatcher、retention、HTTP 的统一启动、监督与 graceful shutdown
```
...
`SINAN_CONTROL_PLANE_ACCOUNTS` 的原始环境变量必须存在且非空；按逗号分隔、trim 并过滤空项后的账户集合可以为空，此时该 principal 不能读写任何账户对象，只能订阅全局脱敏的 `system.event / deadletter.summary`。所有 event capacity、limit、message bytes 和 duration 配置都必须为正数。当前没有 Gateway listener 或 recovery worker 的 `SINAN_*` 运行配置，因为对应组件尚未接入 `main`；不能仅凭库内已有 config 类型宣称进程已经支持这些环境变量。
...
上表是完整目标 workflow 语义。当前第八里程碑的 `POST /trade-intents` 只负责 durable intake，因此只会返回 `202 / ACCEPTED` 或幂等的 `200 / DUPLICATE`；`RISK_BLOCKED / REJECTED` 两行要等下一里程碑的 Risk→Execution workflow processor 接入后才可由 POST 返回。当前已持久化的 rejected RiskResult 会在 `GET /trade-intents/:intent_id` 中派生为 `RISK_BLOCKED`，但不会把一次 POST replay 伪装成新的同步风控响应。
...
ACCEPTED
  ...
  → 后续 hard-risk 只能由 Trading Core 使用同一可信读快照组装完整 RiskRequest；intake handler 不得用 requested_at 伪造 StrategyDecision.timestamp，也不得在缺少 policy / capacity / full-set watermark 时假装已完成风控
  → 不代表 broker 已成交
...
TradeIntent missing account_id / idempotency_key → schema validation failed
...
global TransportEvent summary omits message/metadata/raw payload/remote/session/message identity/parser detail; durable fact retains only diagnostics currently carried by TransportEvent, while raw/type/schema evidence remains an explicit gap
