...
`TransportAck` 以 23.1 的 payload 定义为权威规格。`ACCEPTED` 表示通过最小 envelope / schema 校验并进入 handler queue；`DUPLICATE` 表示 `message_id` 已见且不重复入队；`REJECTED` 表示可关联的消息被拒绝，此时 `reason` 必填。三种状态都不表示 command 已执行或 execution state 已变更。
...
transport.ack
  → TransportAck
  → ACCEPTED: 通过最小 envelope / schema 校验并进入 handler queue
  → DUPLICATE: message_id 已见，不重复入队
  → REJECTED: 可关联消息被拒绝，reason 必填
...
```text
transport.ack
  → status=ACCEPTED 表示 WireMessage envelope 已解析、schema 已通过、进入对应 handler queue
  → status=DUPLICATE 表示 message_id 已处理或已接纳，不重复入队
  → status=REJECTED 表示可关联消息被协议层拒绝，reason 必填
  → 不表示 command 已执行
...
```text
migration 文件命名：V0001__init.sql、V0002__state_store_schema.sql、V0003__execution_durability.sql、V0004__reconciliation_durability.sql、V0005__gateway_delivery_durability.sql
...
execution_client_sessions
  ...
  → max_inflight_commands 是 hello.accepted 时协商并在 session epoch 内不可变的限制
